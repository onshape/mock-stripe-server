...
function Subscriptions(stripe) {
  ...
  self.createSubscription = function(req, res, next) {
    ...
    for (const item of items) {
      const plan = stripe.store.getPlan(req.authorization.identity, item.plan);
      if (!plan) {
        return stripe.errors.invalidRequestError({
          statusCode: 400,
  ...
  self.updateSubscription = function(req, res, next) {
    ...
    const customer = stripe.store.getCustomer(req.authorization.identity, subscription.customer);
    ...
    if (req.body.quantity && subscription.items.data.length) {
      items.push({
    ...
    const prorationItems = items.filter(function(item) {
      ...
      item.plan = stripe.store.getPlan(req.authorization.identity, item.subscriptionItem.plan);
